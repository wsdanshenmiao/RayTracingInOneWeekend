...
namespace DSM {
    ...
    public:
        ...
        void Render(const Hittable& world, const Hittable& light);
        const Image& GetImage() const { return m_Image; }

    private:
        void UpdateCamera();
        Ray GetRay(std::uint32_t x, std::uint32_t y, uint32_t, uint32_t sy) const;
        Color GetRayColor(const Ray& ray, int depth, const Hittable& world, const Hittable& light)const;
        Vector2f GetSquare() const;
        Vector2f GetSquareStratified(uint32_t sx, uint16_t sy) const;
